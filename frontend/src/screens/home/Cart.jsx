   ...
   <motion.div
      ...
      animate={{ opacity: 1 }}className='my-container mt-28'>
    {
        ...
        <div className="table-container ">
            <table className='w-full'>
                ...
                <tbody>
                    {
                        cart.map(item=>{
                            const total = currency.format(discount(item.price,item.discount) * item.quantity,{code:"USD"}) 
                            return (
                                <tr key={item._id} className='even:bg-gray-50'>
                                    <td className='td'>
                                        <img className='w-12 h-12 object-cover rounded-full' src={`/images/${item.image1}`} alt={item.title} />
                                    </td>
                                    ...
                                    <td className='td font-bold text-gray-900'>{currency.format(discount(item.price,item.discount),{code:"USD"})}</td>
                                    <td className='td '><Quantity theme="indigo" quantity={item.quantity} inc={()=>inc(item._id)} dec={()=>dec(item._id)}/></td>
                                    <td className='td font-bold'>{total}</td>
                                    <td className='td'>
                                        <span onClick={()=>remove(item._id)} className=' cursor-pointer'><BsTrash className='text-rose-600
                                        ' size={20}/></span>
                                    </td>
                                </tr>
                            )
        ...
        <div className='bg-indigo-50 p-4 flex justify-end'>
            <div>
                <span className='text-lg font-semibold text-indigo-800 mr-10 mt-5 rounded-md'>{currency.format(total,{code:"USD"})}</span>
            <button className='btn bg-indigo-600 text-sm font-medium py-2.5' onClick={pay}>{res.isLoading ? "Loading..." : "Checkout"}</button>
            </div>
        </div>
        </>
  ): (
